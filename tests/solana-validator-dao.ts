...
describe("solana-validator-dao", () => {
  ...
  it("Create a governance!", async () => {
    ...
    connection.confirmTransaction(await connection.requestAirdrop(owner.publicKey, 1000 * LAMPORTS_PER_SOL));
    ...
    const councilMint =  mint = await createMint(
      connection,
      owner,
      owner.publicKey,
      null,
    ...
    const realmAddress = await governance.withCreateRealm(
      ...
      owner.publicKey,
      councilMint,
      new governance.MintMaxVoteWeightSource({value: new BN(1.5)}),
      new anchor.BN(1),
    );
    ...
    connection.confirmTransaction(await connection.requestAirdrop(native_treasury, 1000 * LAMPORTS_PER_SOL));
  });
});
